...
## Loyalty Rewards Invariants (2026-04-23)

> Added as part of the Loyalty Rewards Cleanup workstream (T1-T11). These invariants describe the **target end-state** after the cleanup ships. See `docs/features/loyalty.md` for the full spec and `~/.claude/projects/-Users-brianlewis/memory/loyalty-cleanup.md` for the migration plan.
>
> Pre-cleanup audit finding: 0 of 154 paid orders had a linked customer; 0 `LoyaltyTransaction` rows in the dev DB; one customer had 530 unexplained points. These invariants exist so the fix cannot regress silently.

| # | Invariant | Where Enforced | Test Trigger |
|---|-----------|----------------|--------------|
| INV-LOYALTY-1 | Every paid order with a linked `customerId` AND `settings.loyalty.enabled` AND a valid `loyaltyProgramId` creates exactly ONE `LoyaltyTransaction(type='earn', orderId=<order>)`. DB-level partial unique index `(orderId) WHERE type='earn' AND deletedAt IS NULL` is the backstop. | `commit-payment-transaction.ts` (pre-compute) + `run-payment-post-commit-effects.ts` (outbox drain); partial unique index in schema; shared across pay, close-tab, pay-all-splits, online/checkout, /loyalty/earn | Pay an order with a linked customer → verify exactly one `LoyaltyTransaction(type='earn', orderId=X)` exists. Retry the same route → expect HTTP 409 `alreadyEarned: true`, no new row. |
| INV-LOYALTY-2 | Refund or void of a paid order with a linked customer reverses the earn by writing exactly one `LoyaltyTransaction(type='adjust', points=-N, metadata.reversesTransactionId=<earnId>)`. Repeated invocations are idempotent — same reversal checked by `metadata.reversesTransactionId` uniqueness. `Customer.loyaltyPoints` and `Customer.lifetimePoints` both decrement by `N`. | `refund-payment/route.ts`, `void-payment/route.ts` (reversal helper); see `refund-void.md` | Pay order with linked customer → void payment → verify single adjust row with negative points + matching `reversesTransactionId` + `Customer.loyaltyPoints` decremented. Call void-payment twice → second call produces no new adjust row. |
| INV-LOYALTY-3 | Direct writes to `Customer.loyaltyPoints` or `Customer.lifetimePoints` outside the canonical engine + outbox are FORBIDDEN. Every balance change MUST be accompanied by a `LoyaltyTransaction` row. Manual admin adjust goes through `/api/loyalty/earn` (or a future `/api/loyalty/adjust`) that writes a ledger row. | Code review + grep for `loyaltyPoints: { increment:` / `loyaltyPoints: { decrement:` / `SET "loyaltyPoints"` outside `run-payment-post-commit-effects.ts`, `/api/loyalty/earn`, `/api/loyalty/redeem`, and reversal helpers | Grep the codebase for direct writes to `Customer.loyaltyPoints` / `Customer.lifetimePoints` → every occurrence MUST be inside the canonical engine, outbox worker, `/api/loyalty/earn`, `/api/loyalty/redeem`, or reversal helper. Any other match = regression. |
| INV-LOYALTY-4 | Online checkout (`POST /api/online/checkout`) earns IDENTICAL points to `POST /api/orders/[id]/pay` for the same cart/customer/settings. There is no parallel/flat/`Math.floor(orderTotal)` fallback path. `customer-upsert.ts:accrueOnlineLoyaltyPoints` is removed (or is a thin wrapper that delegates to the canonical engine). | `online/checkout/route.ts` invokes the canonical engine; `customer-upsert.ts` has no divergent earn implementation | Place the same cart (item, quantity, tax, customer) via `/api/orders/[id]/pay` and via `/api/online/checkout` → verify `LoyaltyTransaction.points` is identical for both orders. Grep for `Math.floor(orderTotal)` in `customer-upsert.ts` returns zero matches. |
| INV-LOYALTY-5 | Tier promotion/demotion is deterministic from `Customer.lifetimePoints` crossing `LoyaltyTier.minimumPoints` thresholds (highest `minimumPoints <= lifetimePoints` wins). Promotion fires on earn; demotion fires on reversal. Both run in the same DB transaction as the balance change. Re-running tier-check on an unchanged `lifetimePoints` produces no change. | `run-payment-post-commit-effects.ts` (promotion loop, lines ~446-465); reversal helper (demotion); `/api/loyalty/tier-check` (idempotent re-evaluate) | Configure tiers Silver=500, Gold=2000. Customer at 480 lifetime points earns 25 points → `loyaltyTierId` advances to Silver. Void the source payment (25-point reversal → 480) → `loyaltyTierId` demotes to the tier whose `minimumPoints <= 480`. Call `/api/loyalty/tier-check` twice back-to-back → second call makes no change. |
| INV-LOYALTY-6 | Process death between payment commit and loyalty write does NOT lose the earn. The post-commit loyalty write is enqueued to a durable outbox inside the payment transaction (same pattern as `add-ha-payment`). The outbox worker drains with retries, and the partial unique index makes retries safe. Fire-and-forget `void ... .catch(...)` at the old `run-payment-post-commit-effects.ts:415` call site is REPLACED by the outbox enqueue. | `commit-payment-transaction.ts` (outbox enqueue inside tx); outbox worker (`outbox-drain.ts` or equivalent); partial unique index on `LoyaltyTransaction(orderId) WHERE type='earn'` | Pay an order → kill the server process immediately after the payment commit (before the outbox drain runs). Restart the server → verify the outbox worker drains the pending earn and the `LoyaltyTransaction` row exists. Metric `orders_paid_with_customer_without_loyalty_txn_within_30s` (INV-LOYALTY observability) stays at zero. |

> **Consequences of violating any loyalty invariant:** customer complaints ("I paid and didn't get my points"), fraud surface (manual points without a ledger), or tier drift (customer shows Platinum but `lifetimePoints` doesn't support it). These map 1:1 to the failure modes in the 2026-04-23 audit.
