...
| T-047 | Wire dispatchOpenOrdersChanged into void/delete route — `/api/orders/[id]` DELETE (void) should fire `dispatchOpenOrdersChanged(locationId, { trigger: 'voided' })`. Currently only create + pay routes fire it. | PM: Orders | PM: Orders | 2026-02-09 | P2 | Skill 248 gap. File: src/app/api/orders/[id]/route.ts |
