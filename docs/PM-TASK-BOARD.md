...
| T-045 | Organize & build Settings admin pages — Audit all settings in `src/lib/settings.ts` (BarTabSettings, PaymentSettings, DualPricingSettings, PriceRoundingSettings, TipSettings, POSDisplaySettings, etc.) and build proper admin UI sections for each at `/settings`. Currently many settings have TypeScript interfaces and defaults but NO admin page to adjust them. Consolidate overlap between Order Type workflow rules and location-level settings (e.g., `requireCardForTab` vs Order Type `requirePaymentBeforeSend`). Ensure owners can configure: Bar Tabs (card requirements, timeout), Payments (pre-auth amounts, tip suggestions, walkout recovery), Dual Pricing (cash discount %), Tax, Tips (tip-out rules, tip guide basis), POS Display, Receipts. Remove duplicate/conflicting settings. | PM: Settings | PM: Orders | 2026-02-08 | P1 | Many settings only accessible via API/defaults. Key file: `src/lib/settings.ts`. Existing settings page: `/src/app/(admin)/settings/page.tsx`. Bar Tab settings have NO UI at all. Payment settings partially built. |
...
| T-048 | KDS device browser version audit — Check all KDS devices for Chrome version. Devices < 108 cannot render Tailwind v4 colors even with oklch transpilation. Add browser version display to KDS admin screen page. | PM: Hardware | PM: KDS | 2026-02-10 | P2 | Discovered during KDS white screen debugging. Device info stored in KDSScreen.deviceInfo JSON. |
...
| T-050 | Investigate Tailwind v4 `@tailwindcss/postcss` dev vs prod optimization gap — In dev mode, optimization is skipped entirely (no Lightning CSS transpilation). Consider setting `optimize: true` in postcss config to ensure consistent CSS output across environments. | PM: Development-RnD | PM: KDS | 2026-02-10 | P3 | Currently relying on `@csstools/postcss-oklab-function` plugin to handle all oklch transpilation. This works but means dev and prod CSS could differ in other ways. |
...
| T-071 | Online ordering middleware routing — Add `/:orderCode/:slug` path matching to POS middleware for customer-facing pages. Resolve orderCode to CloudLocation, validate slug matches. | PM: Mission Control | PM: Mission Control | 2026-02-12 | P2 | Blocked by online ordering UI build. Infrastructure ready (Skill 336). |
| T-072 | Online ordering pages — Build customer-facing menu, cart, checkout at `ordercontrolcenter.com/{code}/{slug}`. Responsive mobile-first design. | PM: Mission Control | PM: Mission Control | 2026-02-12 | P2 | Blocked by T-071. Major feature. URL infrastructure ready. |
| T-073 | QR code generation for order codes — Generate QR codes from venue orderCode for table tents, receipts, business cards. Display/download in MC location detail. | PM: Mission Control | PM: Mission Control | 2026-02-12 | P3 | Nice-to-have once online ordering is live. |
...
| T-075 | Add environment field to CloudLocation — Add `environment` enum (development/staging/production) so deploy modal can filter/group locations by environment type | PM: Mission Control | PM: Mission Control | 2026-02-12 | P3 | UX improvement for deploy workflow. Currently admin manually picks locations. |
...
|----|------|-------------|------|-------|
| T-031 | Remove production console logging from Floor Plan hot paths | PM: Floor Plan | 2026-02-20 | Removed console.error from 5 hot-path handlers in UnifiedFloorPlan.tsx. Commit `423febb` |
| T-032 | Replace Math.random() with deterministic table placement | PM: Floor Plan | 2026-02-20 | ALREADY DONE — deterministic grid placement was already in place. Verified by audit. |
| T-033 | Add API failure rollback + user notifications to Floor Plan editor | PM: Floor Plan | 2026-02-20 | toast.error + optimistic rollback added to 5 handlers. response.ok check in handlePointerUp. Commit `423febb` |
| T-044 | Verify VOID/COMP stamps render on FloorPlanHome | PM: Orders | 2026-02-20 | ALREADY DONE — stamps, voidReason, wasMade all correctly wired end-to-end. Verified by audit. |
| T-047 | Wire dispatchOpenOrdersChanged into void/delete route | PM: Orders | 2026-02-20 | ALREADY DONE — both comp-void and void-tab routes already fire the event. Verified by audit. |
...
| T-079 | Handle partial payment approval flow | PM: Payments | 2026-02-20 | Void & Retry now calls onCancel() after void to auto-return to method selection; Payment Progress banner shown when pendingPayments > 0; remaining balance math verified correct. Commit `ef9eb04` |
