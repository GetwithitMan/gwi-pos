...
| 433 | ANDROID-RETROFIT-LIFECYCLE | DONE | Android / Network | 430 | Replaced per-request URL rewriting in AuthInterceptor with ApiServiceHolder singleton (AtomicReference<GwiApiService?>) that rebuilds Retrofit when nucBaseUrlFlow emits new URL. TokenProvider gets nucBaseUrlFlow: StateFlow<String?>, seeded from prefs, emits on setNucBaseUrl(). AuthInterceptor stripped to auth header only (~15 lines). NetworkModule drops provideRetrofit/provideGwiApiService — Hilt auto-provides ApiServiceHolder. 5 consumers (OrderRepository, AuthRepository, BootstrapWorker, SyncWorker, OutboxWorker) swapped apiService → apiHolder.api (25 call sites). Pre-pairing: api getter throws IllegalStateException. Commits: a945e61 |
| 434 | ANDROID-OFFLINE-UX-GUARDRAILS | DONE | Android / UI / Sync | 433 | Comprehensive offline UX: ConnectionBanner (red full-width, 3s delay, auto-hide on reconnect), OutboxDetailSheet (ModalBottomSheet sync queue with status dots — Amber/Blue/Orange/Red — retry all failed, clear dead letters), PosHeader dead-letter badge (red count badge, clickable amber pending badge opens sheet), payment blocked offline (hard error), kitchen send warned offline (proceeds via outbox). OutboxDao: 4 new queries (getDeadLetters, getAllPendingAndFailed, resetFailedForRetry, deleteDeadLetters). OrderViewModel: connectionBannerJob with 3s delay, outbox sheet lifecycle. Commits: a945e61 |
