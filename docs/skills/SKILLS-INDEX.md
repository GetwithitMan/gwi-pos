...
| 289 | Edit Item Modal (ItemSettingsModal) | DONE | Menu | 217 | Comprehensive Edit Item modal (5 tabs: Basics, Display, Kitchen, Availability, Tax), image upload, collapsible ingredient cost breakdown, card price read-only, auto-open for new items, live sync. Basics tab includes size options / quick pick toggles and per-option showOnPos visibility (Skill 456). |
...
| 454 | Pricing Options — Size Variants | DONE | Menu / Inventory / Orders | 125, 211, 215, 289 | Size variants for food items (S/M/L, Bowl/Cup). Basics tab toggle, prep item linking, costAtSale snapshot, PMIX/Sales report grouping by size. Android sync (DB v21). Superseded by Skill 456 (removed max-4 creation limit, unified Quick Pick into Basics tab). |
| 455 | Quick Pick Labels | DONE | Menu / POS | 454 | Label-only quick picks (Mild/Medium/Hot). showAsQuickPick=true discriminator. Optional prep item linking for inventory. Kitchen chit prints `** LABEL **`. Superseded by Skill 456 (Quick Pick tab removed, merged into Basics tab). |
| 456 | Unified Size Options + Quick Pick + showOnPos | DONE | Menu / POS / Android | 454, 455 | Merged size options and quick picks into two mutually-exclusive checkboxes on Basics tab. Added per-option `showOnPos` toggle (eye icon) to control POS button visibility (max 4 display cap). Removed Quick Pick tab and QuickPickTab.tsx. Removed max-4 creation limit (unlimited options, display capped at 4). POS display (FloorPlanMenuItem, BartenderView) filters by `option.showOnPos` instead of `group.showAsQuickPick`. Schema: `showOnPos Boolean @default(false)` on PricingOption. Android: DTO + entity + DAO query updated, DB v22. 17 files modified + 1 deleted (POS), 6 files modified (Android). Commits: POS `5e2b1d8`, Android `c7672fe`. |
...
| 461 | PAX A3700 CFD — NUC Backend | DONE | Hardware, Customer Display | 389, 120 | Schema: CFD_DISPLAY enum, Terminal CFD fields (cfdTerminalId/cfdIpAddress/cfdConnectionMode), CfdSettings model (tip/signature/receipt/tab config). emitToTerminal() for targeted terminal-room socket dispatch. 5 dispatchCFD* functions accept cfdTerminalId param. POST/DELETE /api/hardware/terminals/[id]/pair-cfd. GET/PUT /api/hardware/cfd-settings. nuc-pre-migrate.js: 4 idempotent migration cases. NUC commit: 54b97da. |
| 462 | GWI CFD Android App (PAX A3700) | DONE | Hardware, Customer Display, Android | 461 | Full Android project at gwi-cfd/. Stateless kiosk app (no Room, no WorkManager). Socket-driven 8-state state machine: Idle/Order/TipPrompt/SignaturePrompt/Processing/Approved/Declined/ReceiptOptions. CfdSocketManager joins terminal:{id} room; 3 outbound emitters (tipSelected/signatureDone/receiptChoice). CfdViewModel: receipt countdown timer, auto-idle timers (Approved 4s, Declined 3s), calculateTipCents(). 8 Compose screens with animations. AppNavigation: AnimatedContent transitions + amber disconnection banner. Android commit: 9cc8123. |
...
| 492 | Berg Liquor Controls Integration | DONE | Inventory / Integrations / Hardware | 52, 128 | Two-tier opt-in: Tier 1 = reports-only (BergPluMapping, PLU CRUD, comparison report UI with manual variance mode); Tier 2 = hardware (BergDevice + BergDispenseEvent, berg-bridge.ts RS-232 serial service, STX/ETX state machine, HMAC auth, idempotency SHA256 window, BEST_EFFORT + REQUIRES_OPEN_ORDER modes, terminalId→offlineTerminalId order scope). 7 Prisma enums. PM2 berg-bridge app. NTP auto-fix on startup. Exponential reconnect backoff 1s→30s. Sprints B+C (settings UI completion + full reporting) pending. SPEC-492. |
...
| Skill | Name | What Was Built |
|-------|------|----------------|
| 482 | PAX A6650 Feature Sprint | 8-phase sprint for the gwi-pax-a6650 handheld: per-terminal IP printer assignment (kitchenPrinterId/barPrinterId on Terminal, SyncMeta, PrinterManager fallback chain); QuickBar Editor (server-synced per-employee, ModalBottomSheet with draggable chips, max 12, Reset to default); Dead Letter Actionable Modal (auto-opens on 0→N transition, Retry re-enqueues OutboxWorker, Clear with confirmation); Item Long-Press Actions (Note/Comp/Void via ItemActionsSheet, PermissionEvaluator, POST /api/orders/[id]/items/[itemId]/note); Diagnostics Screen (long-press hidden, SyncMeta/DB/Outbox sections, Copy All); delete-item guard + add-item snackbar; login delay 3s→1.2s; Table↔Tab mode switch chip; payment retry button; kiosk cancel socket event. Repos: gwi-pax-a6650 commits `ada799b`+`ff91590`, gwi-pos commit `605f127`, gwi-android-register commits `c6095de`+`a20f565`. See `docs/skills/482-PAX-A6650-FEATURE-SPRINT.md`. |
| 483 | PAX A6650: TABLE_OCCUPIED Stale Table State Fix | Fixed table grid showing "Available" for server-occupied tables, and raw JSON error bubble on tap. Root causes: stale Room cache (order created on another terminal between delta syncs) + no TABLE_OCCUPIED recovery path. Fix: `CreateOrderUseCase` parses BusinessError JSON — on TABLE_OCCUPIED fetches+caches the existing order and navigates to it directly. `TableHomeViewModel.init` fires `refreshOpenOrders()` on entry to correct stale badges within ~1s. `TableUiState.errorMessage` replaces silent TODO with user-facing snackbar for genuine failures. Repo: gwi-pax-a6650 commit `27e232b`. See `docs/skills/483-PAX-A6650-TABLE-OCCUPIED-FIX.md`. |
...
### Android Register — Same System as Web POS + Production Hardening (2026-02-24)
...
| 433 | ANDROID-RETROFIT-LIFECYCLE | DONE | Android / Network | 430 | DynamicBaseUrlInterceptor (OkHttp interceptor with AtomicReference) rewrites scheme/host/port per-request — preserves connection pool. ApiServiceHolder singleton throws NucNotConfiguredException when unpaired. Single Retrofit instance with placeholder base URL. TokenProvider nucBaseUrlFlow seeds interceptor. AuthInterceptor stripped to auth header only. Commits: a945e61, 499d62d |
| 434 | ANDROID-OFFLINE-UX-GUARDRAILS | DONE | Android / UI / Sync | 433 | Dual-signal isOnline (socket + HTTP health check every 10s). ConnectionBanner ("POS offline — orders may not sync"). OutboxDetailSheet (ModalBottomSheet sync queue). Payment blocked offline (hard error). Kitchen send → kitchenQueuedWarning when outbox pending. orderClosedElsewhere detection. 5-min idle timeout → sessionExpired auto-logout. unreconciledPaymentCount red badge. Commits: a945e61, 047a1f7 |
| 435 | ANDROID-SECURITY-HARDENING | DONE | Android / Security / Data | 430 | PIN removed from EmployeeEntity/Room (PCI — never persisted on device). NucNotConfiguredException domain exception (replaces raw IllegalStateException). OrderRepository private api helper wraps → IOException. OutboxWorker + BootstrapWorker catch gracefully. Commits: 5b7c96a, 047a1f7 |
| 436 | ANDROID-OUTBOX-SEQUENCING | DONE | Android / Sync | 430 | OutboxEntry: sequenceNumber + Index("offlineId"). OutboxDao.getRetryable() uses NOT EXISTS subquery for per-order strict sequencing (entry N must complete before N+1). getNextSequenceNumber() auto-increments per offlineId. OutboxWorker parsePayload<T> generic helper with dead-letter on JSON parse failure. Commits: 5b7c96a, 499d62d |
| 437 | ANDROID-UPSERT-PRUNE-DAOS | DONE | Android / Data | 430 | Upsert-then-prune pattern across all DAOs: MenuDao.syncMenuData(), CachedOrderDao.replaceOrderItems() (upsert + deleteStaleOrderItems), replaceOpenOrders protects hasPendingOutbox. EmployeeDao, TableDao, OrderTypeDao, PaymentReaderDao all got syncX() @Transaction methods. BootstrapWorker uses sync methods instead of deleteAll+insertAll. Flicker-free UI. Commits: 5b7c96a, 5cd6894 |
...
| Skill | Name | Status | Domain | Dependencies | Notes |
|-------|------|--------|--------|--------------|-------|
| 454 | Pricing Options — Size Variants | DONE | Menu / Inventory / Orders | 125, 211, 215, 289 | Size variants for food (S/M/L, Bowl/Cup). 3 Prisma models + costAtSale field. Basics tab toggle, prep item linking via IngredientHierarchyPicker, costAtSale fire-and-forget snapshot, PMIX/Sales grouped by size. Inventory link CRUD API (4 endpoints). Android: Room entities, DAO, sync, Compose sheet (DB v21). Superseded by Skill 456 (max-4 removed, unified into Basics tab). |
...
| 460 | Android ModifierSheet Redesign + Ingredient Fix + Performance | DONE | Android / Menu / UI | 459 | Tab-based modifier sheet (square group tabs, list rows, Steps-only mode, running total). Ingredient section uses real `MenuItemIngredient` data (not pre-mod heuristic). Bootstrap serves `ingredientLinks[]` per menu item. New `IngredientLinkInfo` domain model; DB v34 (MIGRATION_33_34). `buildRequests()` plain-name fix eliminates "NO No Lettuce" double-prefix. Performance: Dialog→ModalBottomSheet; N+1 fix via `getModifiersByGroups()` batch; parallelized child-group loading (coroutineScope+async+ConcurrentHashMap); snapshotFlow debounce 50ms; background prefetch; cache warming at category selection; `loadingMenuItemId` tap-feedback; All mode removed (Steps only). POS commit: `1079272`. Android commits: `9094233`, `2b71fc8`, `a0ece57`, `a478774`. |
| 461 | PAX A3700 CFD — NUC Backend | DONE | Hardware / Customer Display | 389, 120 | Schema: CFD_DISPLAY enum, Terminal CFD fields, CfdSettings model. emitToTerminal() targeted dispatch. 5 dispatchCFD* functions with cfdTerminalId param. POST/DELETE pair-cfd route. GET/PUT cfd-settings route. nuc-pre-migrate.js 4-case migration. NUC commit: 54b97da. |
| 462 | GWI CFD Android App (PAX A3700) | DONE | Hardware / Customer Display / Android | 461 | Full Android project (gwi-cfd/). Stateless kiosk, socket-driven. 8-state CfdScreenState machine. CfdSocketManager (terminal:{id} room). CfdViewModel (timers, countdown, calculateTipCents). 8 Compose screens + AppNavigation. Android commit: 9cc8123. |
| 463 | CFD Phase 3 — Register Wiring + Pairing | DONE | Hardware / Customer Display / Android | 461, 462 | NUC: bootstrap returns terminalConfig{terminalId,locationId,cfdTerminalId}+cfdSettings; socket-server bidirectional relay (6 reg→CFD + 3 CFD→reg events); batch route auto-dispatches cfd:show-order from OrderState projection. Android register: BootstrapWorker stores cfdTerminalId+7 CfdSettings SyncMeta keys; PaymentManager pre-tap tip flow (collectCfdTip race-free coroutineScope/async, 60s timeout); SocketEvent.CfdTipSelected; SocketManager relays cfd:tip-selected; gratuity→Datacap. gwi-cfd: real CfdPairingScreen (OkHttp bootstrap call, stores 4 credentials); build fixes (kotlinOptions→compilerOptions, appcompat, animateItem, theme). NUC commit: 995cb03. Android commit: 795dd66. CFD commit: d17a50b. |
| 464 | Android Tip Adjustment + Edit Modifiers | DONE | Android / Payments | 463 | Tip adjustment after card payment: `TipAdjustmentSheet` (MoneyTextFieldState, manager PIN, Confirm/Cancel); NUC `adjust-tip` route calls `DatacapClient.adjustGratuity()` before DB update; `AdjustTipRequest` DTO + `GwiApiService` PATCH endpoint; wired via `VoidPaymentSheet` "Adjust Tip" → `OrderSheets`. Edit modifiers on pending items: `OrderItemControls` tap → `onEditModifiers()` propagated through `OrderPanel` → `OrderMainContent` → `showEditItemModifiers()`; `ModifierSheet.initialModifiers` param + `LaunchedEffect` pre-populates existing selections (ingredients, groups, child groups). NUC commit: `e100bb0`. Android commit: `ea71b19`. |
| 465 | Android Refund Flow | DONE | Android / Payments | 464 | `RefundSheet`: MoneyTextFieldState amount (capped at payment total), required reason field, manager PIN, red "Process Refund" CTA. `OrderStateTypes.OrderSheet.Refund(paymentId, maxAmountDollars, paymentMethod)`. `RefundPaymentRequest/Response/Data` DTOs. `GwiApiService.refundPayment()` POST. `OrderViewModel.showRefund/dismissRefund/processRefund` (PIN verify → API). `VoidPaymentSheet` gets red "Refund" button on each non-voided payment row. `OrderSheets` Refund branch: dismiss void → open refund. NUC auto-resolves `paymentReaderId`, calls Datacap `emvReturn` for card refunds; cash is DB-only; partial/full; proportional tip reduction. NUC commit: `27a9051`. Android commit: `62051c2`. |
| 466 | Android Gift Card Payment | DONE | Android / Payments | 465 | Gift card as 3rd payment method. `GiftCardData` + `GiftCardLookupResponse` DTOs; `giftCardNumber: String?` on `PayOrderRequest`. `GwiApiService.lookupGiftCard()` GET. `PayOrderUseCase.payGiftCard()` → `payOrder(method=gift_card)`. `OrderViewModel`: gift card state (number/balance/id/isChecking/Error); `checkGiftCardBalance()` + `payGiftCard()` + `resetGiftCardState()`. `PaymentSheet` 3-way toggle (Card / Cash / Gift Card); card# input → "Check Balance" → green balance display → amount pre-filled to `min(balance, remaining)` → "Apply Gift Card" button. Android commit: `78fdb35`. |
| 467 | Android Card-Insert Detection UI | DONE | Android / Hardware | 461, 462, 463 | UI scaffolding for VP3350 card-insert detection. `SocketEvent.CardDetected` + `terminal:card-detected` event constants in `SocketEvents.kt`. `OrderSyncController` pass-through branch. `OrderUiEvent.CardDetected`. `OrderViewModel.collectCardDetected()` observes flow → snackbar "Card detected — start a tab?" with "Open Tab" action. `OrderScreen` snackbar action fires `openTabFromCardDetect()`. `StationHardwareSheet`: debug-only "Simulate Card Insert" button. VP3350 USB persistent read loop not yet implemented (UI scaffolding only). Android commit: `f1e7991`. |
| 468 | Android Payment Result State + My Tips Screen | DONE | Android / Payments / Tips | 466, 250 | Payment result state: `paymentApprovedAmountCents: Long?` + `paymentDeclineReason: String?` in `OrderUiState`; `clearPaymentResult()` helper; `payCash`/`payCard` set `approvedAmountCents` on success (sheet stays open to show result). `PaymentSheet.kt` major rewrite (+865 lines). My Tips employee view: new `ui/tips/` package — `MyTipsScreen` (tip history list, summary cards total/cash/card, date filter), `MyTipsViewModel` (GwiApiService tips endpoint, pagination), `TipEntrySheet` (detail per entry). `Screen.MyTips(employeeId)` route. `AppNavigation` composable. `PosHeader` "My Tips" action → `onNavigateToMyTips`. Android commit: `da56a18`. |
| 469 | Tab Nickname (tabName vs tabNickname) | DONE | Android / Tabs / API | 468 | Android + POS implementation. Card name retained as `tabName`, bartender-assigned display name as `tabNickname`. DB migration 34→35. `CachedOrderEntity.tabNickname`, `NewTabDialog` reworked — card present = read-only card name + editable "Known As" field; name-only = "Known As" required. Tab list shows nickname-first with card name as subtitle. POS: `PUT /api/tabs/[id]` accepts `tabNickname`. |
| 470 | Pending Tips + My Tips Screen (Android) | DONE | Android / Tips | 468, 250 | New hamburger menu screen with two tabs: Pending Tips (closed $0-tip card payments awaiting paper receipt entry) and My Tips (recorded tips, editable). `TipEntrySheet` with % chips showing "20% • $9.60", custom amount, reason dropdown, NEW/EDIT modes. Self-service, no permission gate for own orders. Files: MyTipsViewModel.kt, MyTipsScreen.kt, TipEntrySheet.kt, GwiApiService.kt, Screen.kt, AppNavigation.kt, OrderScreen.kt, OrderMainContent.kt, PosHeader.kt. |
| 471 | Self-Service Tip Adjustment API | DONE | Tips / API | 250, 470 | `GET /api/tips/pending-tips` — closed card payments with tipAmount=0 for requesting employee. `GET /api/tips/recorded-tips` — closed card payments with tipAmount>0 for requesting employee. Self-service path in `POST /api/tips/adjustments` — allows own-order tip entry without TIPS_PERFORM_ADJUSTMENTS permission. `performTipAdjustment()` now also updates `Payment.tipAmount` and `Payment.totalAmount` in DB. |
