...
| 460 | Android ModifierSheet Redesign + Ingredient Fix | DONE | Android / Menu / UI | 459 | Tab-based modifier sheet (square group tabs, list rows, Steps/All toggle, running total). Ingredient section uses real `MenuItemIngredient` data (not pre-mod heuristic). Bootstrap now serves `ingredientLinks[]` per menu item. New `IngredientLinkInfo` domain model; DB v34 (MIGRATION_33_34). `buildRequests()` plain-name fix eliminates "NO No Lettuce" double-prefix. POS commit: `1079272`. Android commit: `9094233`. 12 files total. |
