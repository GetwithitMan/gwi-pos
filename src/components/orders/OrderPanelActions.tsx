...
import { PaymentProcessorView } from './PaymentProcessorView'
...
export const OrderPanelActions = memo(function OrderPanelActions({
  ...
  // customTip/showCustomTip state moved into PaymentProcessorView
  ...
  useEffect(() => {
    if (autoShowPayment && orderId && terminalId && employeeId) {
      ...
      setTipAmount(0)
      // customTip/showCustomTip reset handled by PaymentProcessorView unmount
      onAutoShowPaymentHandled?.()
    }
  ...
  // Suggested tip percentages and custom tip handled by PaymentProcessorView
  ...
  if (showPaymentProcessor) {
    return (
      <PaymentProcessorView
        ...
        datacap={datacap as any}
        onCancel={handleCancelPayment}
        onCollect={handleCollectPayment}
        onPayCash={onPay ? (method: 'cash') => {
          setShowPaymentProcessor(false)
          onPay(method)
        } : undefined}
