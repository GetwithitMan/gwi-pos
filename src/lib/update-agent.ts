...
async function updateComponents(): Promise<ComponentUpdateResult> {
  ...
  try {
    ...
    // Patch heartbeat.sh to include nucReadiness if not already present
    // heartbeat.sh is generated by the installer with embedded secrets — we can't
    // regenerate it, but we CAN inject the nucReadiness block before the BODY= line.
    try {
      ...
      if (existsSync(hbPath)) {
        ...
        if (!hbContent.includes('nucReadiness') && !hbContent.includes('NUC_READINESS')) {
          // Inject nucReadiness gathering + add to payload
...
if [ -n "$NUC_READINESS_RAW" ] && echo "$NUC_READINESS_RAW" | jq empty 2>/dev/null; then
  ...
  NUC_READINESS_JSON=$(jq -c '{localDb:true,neonReachable:.neonReachable,neonSchemaVersion:.observedVersion,seedVersion:null,baseSeedPresent:.syncReady,schemaBehind:false,schemaAhead:false,syncWorkers:.syncReady,expectedSchemaVersion:.expectedVersion,observedNeonSchemaVersion:.observedVersion,schemaRecheckCount:.retryCount,readinessLevel:(if .syncReady then "ORDERS" else "BOOT" end)}' /opt/gwi-pos/state/sync-status.json 2>/dev/null || echo 'null')
fi
`
          // Insert before the BODY= line and add nucReadiness to the jq payload
          let patched = hbContent.replace(
            /^(BODY=\$\(jq -nc)/m,
            nucReadinessBlock + '\n$1'
          )
          // Add nucReadiness arg and field to jq
          ...
          log.info('[update-agent] Patched heartbeat.sh with nucReadiness support')
          updated = true
        }
      }
    } catch (patchErr) {
      log.warn('[update-agent] Heartbeat.sh patch failed (non-fatal):', patchErr instanceof Error ? patchErr.message : patchErr)
    }
