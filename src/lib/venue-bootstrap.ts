        ...
        try {
          ...
          if (config.stationRole === 'backup') {
            ...
            if (!neonState && tableCount === 0) {
              ...
              log.info('Neon DB is empty — running auto-repair via schema.sql (full DDL apply)')
              try {
            ...
            // UNVERIFIED backfill: Tables exist but no _venue_schema_state — likely
            // Vercel ran prisma db push. We create the state table but mark it as
            // UNVERIFIED because we have no proof the actual schema matches
            // EXPECTED_SCHEMA_VERSION. The schema may be partially applied, from a
            // different version, or missing migrations. syncContractReady will still
            // depend on core table + enum checks from buildReadiness(), but monitoring
            // should flag this venue for manual verification.
            log.warn(
              { tableCount, expectedSchema: EXPECTED_SCHEMA_VERSION },
              'Neon DB has tables but no _venue_schema_state — backfilling as UNVERIFIED (schema may not match expected version)'
            )
            try {
              await ensureSchemaStateTable(neonClient)
              await writeSchemaState(neonClient, {
                ...
                provisionerVersion: PROVISIONER_VERSION,
                provisionedAt: new Date(),
                provisionedBy: 'nuc-bootstrap-backfill-unverified',
                appVersion: APP_VERSION,
              })
              result.neonSchemaReady = await buildReadiness(neonClient, {
                schemaVersion: EXPECTED_SCHEMA_VERSION,
