...
/**
 * Dispatch CFD charge-card event
 *
 * Sends a card charge request to the CFD terminal's integrated card reader.
 * The CFD will present the payment UI to the customer, collect card/tip/signature,
 * and respond with a cfd:charge-result event.
 */
export function dispatchCFDChargeCard(locationId: string, cfdTerminalId: string | null, data: {
  terminalId?: string
  orderId: string
  invoiceNo: string
  merchantId: string
  ...
  if (cfdTerminalId) {
    void emitToTerminal(cfdTerminalId, CFD_EVENTS.CHARGE_CARD, data).catch((err) => log.error({ err }, 'CFD charge-card dispatch failed'))
  } else {
    void emitToLocation(locationId, CFD_EVENTS.CHARGE_CARD, data).catch((err) => log.error({ err }, 'CFD charge-card dispatch failed'))
  }
}

/**
 * Dispatch CFD cancel-charge event
 *
 * Cancels an in-progress card charge on the CFD terminal.
 * The CFD should abort the reader transaction and return to idle.
 */
export function dispatchCFDCancelCharge(locationId: string, cfdTerminalId: string | null, data: {
  orderId: string
}): void {
  if (cfdTerminalId) {
    void emitToTerminal(cfdTerminalId, CFD_EVENTS.CANCEL_CHARGE, data).catch((err) => log.error({ err }, 'CFD cancel-charge dispatch failed'))
  } else {
    void emitToLocation(locationId, CFD_EVENTS.CANCEL_CHARGE, data).catch((err) => log.error({ err }, 'CFD cancel-charge dispatch failed'))
  }
}
