...
export default function CompReasonsPage() {
  const { employee } = useRequireAuth()

  const crud = useAdminCRUD<CompReason>({
    apiBase: '/api/inventory/comp-reasons',
    locationId: employee?.location?.id,
    resourceName: 'comp reason',
    parseResponse: (data) => data.compReasons || [],
  })
  ...
  const [deleteTarget, setDeleteTarget] = useState<CompReason | null>(null)
  ...
  const handleEdit = (reason: CompReason) => {
    setFormData({
      name: reason.name,
  ...
  const handleToggleActive = async (reason: CompReason) => {
    try {
      const res = await fetch(`/api/inventory/comp-reasons/${reason.id}`, {
        method: 'PUT',
      ...
      console.error('Failed to toggle comp reason:', error)
    }
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <AdminPageHeader
        title="Comp Reasons"
        subtitle="Manage preset reasons for comping items and orders"
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Comp Reason' : 'Create Comp Reason'}
        size="md"
      >
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="e.g., Complimentary for VIP guests and regulars"
              />
            </div>
