  ...
  try {
    ...
    const receiptData = {
      ...
      items: order.items.map(item => ({
        ...
        price: isDualCard ? applyMarkup(Number(item.price)) : Number(item.price),
        itemTotal: isDualCard ? applyMarkup(Number(item.itemTotal)) : Number(item.itemTotal),
        specialNotes: item.specialNotes,
        status: item.status,
        modifiers: item.modifiers.map(mod => ({
          id: mod.id,
          name: mod.name,
          price: isDualCard ? applyMarkup(Number(mod.price)) : Number(mod.price),
