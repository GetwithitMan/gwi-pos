    ...
    for (const location of locations) {
      ...
      if (s.syncOptions.pushSales) {
        try {
          ...
          const existing = await venueDb.sevenShiftsDailySalesPush.findUnique({
          ...
          if (existing?.status === 'pushed') {
            ...
            const pushRecord = await venueDb.sevenShiftsDailySalesPush.upsert({
              where: { locationId_businessDate_revenueType: { locationId: location.id, businessDate, revenueType: 'combined' } },
            ...
            await venueDb.sevenShiftsDailySalesPush.update({
      ...
      if (s.syncOptions.pushTimePunches) {
        ...
        try {
          const { start, end } = getDateRange(businessDate, tz)
          const entries = await venueDb.timeClockEntry.findMany({
          ...
          for (const entry of entries) {
            ...
            try {
              ...
              await venueDb.timeClockEntry.update({
                where: { id: entry.id },
              ...
              const msg = err instanceof Error ? err.message : 'Unknown'
              await venueDb.timeClockEntry.update({ where: { id: entry.id }, data: { sevenShiftsPushError: msg.slice(0, 500) } })
              failedP++
            }
          }
      ...
      if (s.syncOptions.pullSchedule) {
        ...
        try {
          ...
          const schedule = await venueDb.schedule.findFirst({
          ...
          if (!schedule) {
            ...
            for (const shift of shifts) {
              ...
              if (shift.status === 'deleted') {
                const ex = await venueDb.scheduledShift.findFirst({ where: { sevenShiftsShiftId: String(shift.id), deletedAt: null } })
                if (ex) { await venueDb.scheduledShift.update({ where: { id: ex.id }, data: { deletedAt: new Date() } }); deletedS++ }
                continue
              }

              const ex = await venueDb.scheduledShift.findFirst({ where: { sevenShiftsShiftId: String(shift.id) } })
              if (ex) {
                await venueDb.scheduledShift.update({
                  where: { id: ex.id },
                  data: { date: shiftDate, startTime, endTime, breakMinutes: shift.break_minutes ?? 0, notes: shift.notes ?? ex.notes, deletedAt: null },
                })
              } else {
                await venueDb.scheduledShift.create({
                  data: {
