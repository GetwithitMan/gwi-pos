...
export const POST = withVenue(withAuth(async function POST(
  ...
  try {
    ...
    const { locationId, employeeId } = body as { locationId: string; employeeId?: string }
    ...
    // For cover requests with no target, the accepting employee becomes the target
    const updateData: Record<string, unknown> = {
      status: 'accepted',
      respondedAt: new Date(),
    }

    if (requestType === 'cover' && !swapRequest.requestedToEmployeeId && employeeId) {
      updateData.requestedToEmployeeId = employeeId
    }
