...
export const GET = withVenue(async function GET(request: NextRequest) {
  ...
  const menuCursor = request.nextUrl.searchParams.get('menuCursor') ?? undefined
  const menuPageSize = request.nextUrl.searchParams.get('menuPageSize') ? parseInt(request.nextUrl.searchParams.get('menuPageSize')!, 10) : undefined
  const ordersCursor = request.nextUrl.searchParams.get('ordersCursor') ?? undefined
  const ordersPageSize = request.nextUrl.searchParams.get('ordersPageSize') ? parseInt(request.nextUrl.searchParams.get('ordersPageSize')!, 10) : undefined
  const isPaginatingMenu = menuCursor !== undefined || menuPageSize !== undefined
  const isPaginatingOrders = ordersCursor !== undefined || ordersPageSize !== undefined
  const effectiveMenuPageSize = menuPageSize ?? 500
  const effectiveOrdersPageSize = ordersPageSize ?? 100
  const paginationSuffix = `${menuCursor ? `-mc${menuCursor}` : ''}${menuPageSize !== undefined ? `-mps${menuPageSize}` : ''}${ordersCursor ? `-oc${ordersCursor}` : ''}${ordersPageSize !== undefined ? `-ops${ordersPageSize}` : ''}`
  ...
  const syncCacheKey = `sync-bootstrap-${locationId}${paginationSuffix}`
  ...
  const inflightKey = `bootstrap-${locationId}${paginationSuffix}`
  ...
  const [categories, employees, tables, orderTypes, location, paymentReaders, printers, sections, floorPlanElements, cfdSettings, openOrders, taxRules, pizzaToppings, discountRules, voidReasons, compReasons, comboTemplatesRaw] = await Promise.all([
    db.category.findMany({
      ...
      take: isPaginatingMenu ? effectiveMenuPageSize + 1 : undefined,
      ...(isPaginatingMenu && menuCursor ? { cursor: { id: menuCursor }, skip: 1 } : {}),
      orderBy: (isPaginatingMenu ? { id: 'asc' } : { sortOrder: 'asc' }) as any,
    }),
    ...
    adminDb.order.findMany({
      ...
      ...(isPaginatingOrders
        ? {
            take: effectiveOrdersPageSize + 1,
            ...(ordersCursor ? { cursor: { id: ordersCursor }, skip: 1 } : {}),
          }
        : { take: 500 }),
      include: {
        items: {
      ...
      orderBy: isPaginatingOrders ? { id: 'asc' as const } : { createdAt: 'desc' as const },
    }),
  ...
  // ─── Pagination: trim sentinel items and compute cursors ─────────────────
  let menuHasMore = false
  let menuNextCursor: string | null = null
  if (isPaginatingMenu && categories.length > effectiveMenuPageSize) {
    categories.pop()
    menuHasMore = true
    menuNextCursor = categories[categories.length - 1].id
  }

  let ordersHasMorePage = false
  let ordersNextCursor: string | null = null
  if (isPaginatingOrders && openOrders.length > effectiveOrdersPageSize) {
    openOrders.pop()
    ordersHasMorePage = true
    ordersNextCursor = openOrders[openOrders.length - 1].id
  }
  ...
  const responseData = {
      ...
      ...(isPaginatingMenu ? { menuNextCursor, menuHasMore } : {}),
      ...(isPaginatingOrders ? { ordersNextCursor, ordersHasMore: ordersHasMorePage } : {}),
      // Open orders: allows device to rebuild state after mid-service reboot
      // without relying solely on socket catch-up (which may miss events).
