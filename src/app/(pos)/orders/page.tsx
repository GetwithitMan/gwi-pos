...
import { useModifierFlowStore } from '@/stores/modifier-flow-store'
import { usePaymentFlowStore } from '@/stores/payment-flow-store'
import { useDiscountFlowStore } from '@/stores/discount-flow-store'
import { useOrderUiStore } from '@/stores/order-ui-store'
// Backward-compat facade — used only for getState() in imperative callbacks
...
export default function OrdersPage() {
  ...
  // ── Domain stores (split from former god-store) ──
  // Modifier flow store — modifiers, pizza, combo, entertainment, timed rental, weight, age, allergens
  const setShowModifierModal = useModifierFlowStore(s => s.setShowModifierModal)
  const setSelectedItem = useModifierFlowStore(s => s.setSelectedItem)
  const setItemModifierGroups = useModifierFlowStore(s => s.setItemModifierGroups)
  const setLoadingModifiers = useModifierFlowStore(s => s.setLoadingModifiers)
  const setEditingOrderItem = useModifierFlowStore(s => s.setEditingOrderItem)
  const setShowPizzaModal = useModifierFlowStore(s => s.setShowPizzaModal)
  const setSelectedPizzaItem = useModifierFlowStore(s => s.setSelectedPizzaItem)
  const setSelectedPizzaSpecialty = useModifierFlowStore(s => s.setSelectedPizzaSpecialty)
  const setEditingPizzaItem = useModifierFlowStore(s => s.setEditingPizzaItem)
  const setShowComboModal = useModifierFlowStore(s => s.setShowComboModal)
  const setSelectedComboItem = useModifierFlowStore(s => s.setSelectedComboItem)
  const setComboTemplate = useModifierFlowStore(s => s.setComboTemplate)
  const setComboSelections = useModifierFlowStore(s => s.setComboSelections)
  const setShowTimedRentalModal = useModifierFlowStore(s => s.setShowTimedRentalModal)
  const setSelectedTimedItem = useModifierFlowStore(s => s.setSelectedTimedItem)
  const setSelectedRateType = useModifierFlowStore(s => s.setSelectedRateType)
  const setLoadingSession = useModifierFlowStore(s => s.setLoadingSession)
  const setShowEntertainmentStart = useModifierFlowStore(s => s.setShowEntertainmentStart)
  const setEntertainmentItem = useModifierFlowStore(s => s.setEntertainmentItem)
  const setShowWeightModal = useModifierFlowStore(s => s.setShowWeightModal)
  const setWeightCaptureItem = useModifierFlowStore(s => s.setWeightCaptureItem)
  const setPricingPickerItem = useModifierFlowStore(s => s.setPricingPickerItem)
  const setShowAgeVerification = useModifierFlowStore(s => s.setShowAgeVerification)
  const setAgeVerificationItem = useModifierFlowStore(s => s.setAgeVerificationItem)
  const setAgeVerificationCallback = useModifierFlowStore(s => s.setAgeVerificationCallback)
  const setAllergenNotice = useModifierFlowStore(s => s.setAllergenNotice)
  const selectedItem = useModifierFlowStore(s => s.selectedItem)
  const editingOrderItem = useModifierFlowStore(s => s.editingOrderItem)
  const selectedPizzaItem = useModifierFlowStore(s => s.selectedPizzaItem)
  const editingPizzaItem = useModifierFlowStore(s => s.editingPizzaItem)
  const selectedComboItem = useModifierFlowStore(s => s.selectedComboItem)
  const comboTemplate = useModifierFlowStore(s => s.comboTemplate)
  const comboSelections = useModifierFlowStore(s => s.comboSelections)
  const selectedTimedItem = useModifierFlowStore(s => s.selectedTimedItem)
  const selectedRateType = useModifierFlowStore(s => s.selectedRateType)
  const entertainmentItem = useModifierFlowStore(s => s.entertainmentItem)
  const showWeightModal = useModifierFlowStore(s => s.showWeightModal)
  const weightCaptureItem = useModifierFlowStore(s => s.weightCaptureItem)
  const pricingPickerItem = useModifierFlowStore(s => s.pricingPickerItem)
  const showAgeVerification = useModifierFlowStore(s => s.showAgeVerification)
  const ageVerificationItem = useModifierFlowStore(s => s.ageVerificationItem)
  const ageVerificationCallback = useModifierFlowStore(s => s.ageVerificationCallback)
  const allergenNotice = useModifierFlowStore(s => s.allergenNotice)

  // Payment flow store — payment, card tab, split tickets
  const setShowPaymentModal = usePaymentFlowStore(s => s.setShowPaymentModal)
  const setInitialPayMethod = usePaymentFlowStore(s => s.setInitialPayMethod)
  const setOrderToPayId = usePaymentFlowStore(s => s.setOrderToPayId)
  const setPaymentTabCards = usePaymentFlowStore(s => s.setPaymentTabCards)
  const setShowCardTabFlow = usePaymentFlowStore(s => s.setShowCardTabFlow)
  const setCardTabOrderId = usePaymentFlowStore(s => s.setCardTabOrderId)
  const setTabCardInfo = usePaymentFlowStore(s => s.setTabCardInfo)
  const setShowSplitTicketManager = usePaymentFlowStore(s => s.setShowSplitTicketManager)
  const setSplitManageMode = usePaymentFlowStore(s => s.setSplitManageMode)
  const setEditingChildSplit = usePaymentFlowStore(s => s.setEditingChildSplit)
  const setSplitParentToReturnTo = usePaymentFlowStore(s => s.setSplitParentToReturnTo)
  const setPayAllSplitsQueue = usePaymentFlowStore(s => s.setPayAllSplitsQueue)
  const setShowPayAllSplitsConfirm = usePaymentFlowStore(s => s.setShowPayAllSplitsConfirm)
  const setPayAllSplitsTotal = usePaymentFlowStore(s => s.setPayAllSplitsTotal)
  const setPayAllSplitsCardTotal = usePaymentFlowStore(s => s.setPayAllSplitsCardTotal)
  const setPayAllSplitsParentId = usePaymentFlowStore(s => s.setPayAllSplitsParentId)
  const setPayAllSplitsProcessing = usePaymentFlowStore(s => s.setPayAllSplitsProcessing)
  const setPayAllSplitsStep = usePaymentFlowStore(s => s.setPayAllSplitsStep)
  const setOrderSplitChips = usePaymentFlowStore(s => s.setOrderSplitChips)
  const setSplitParentId = usePaymentFlowStore(s => s.setSplitParentId)
  const setSplitChipsFlashing = usePaymentFlowStore(s => s.setSplitChipsFlashing)
  const showSplitTicketManager = usePaymentFlowStore(s => s.showSplitTicketManager)
  const editingChildSplit = usePaymentFlowStore(s => s.editingChildSplit)
  const orderSplitChips = usePaymentFlowStore(s => s.orderSplitChips)
  const splitChipsFlashing = usePaymentFlowStore(s => s.splitChipsFlashing)
  const splitParentId = usePaymentFlowStore(s => s.splitParentId)
  const splitParentToReturnTo = usePaymentFlowStore(s => s.splitParentToReturnTo)
  const payAllSplitsQueue = usePaymentFlowStore(s => s.payAllSplitsQueue)
  const payAllSplitsParentId = usePaymentFlowStore(s => s.payAllSplitsParentId)
  const tabCardInfo = usePaymentFlowStore(s => s.tabCardInfo)
  const orderToPayId = usePaymentFlowStore(s => s.orderToPayId)
  const cardTabOrderId = usePaymentFlowStore(s => s.cardTabOrderId)

  // Discount flow store — discounts, comp/void
  const setShowDiscountModal = useDiscountFlowStore(s => s.setShowDiscountModal)
  const setAppliedDiscounts = useDiscountFlowStore(s => s.setAppliedDiscounts)
  const setItemDiscountTargetId = useDiscountFlowStore(s => s.setItemDiscountTargetId)
  const setShowCompVoidModal = useDiscountFlowStore(s => s.setShowCompVoidModal)
  const setCompVoidItem = useDiscountFlowStore(s => s.setCompVoidItem)

  // Order UI store — tabs, receipts, resend, transfers, notes, shifts, display settings
  const setShowDisplaySettings = useOrderUiStore(s => s.setShowDisplaySettings)
  const setShowTabsPanel = useOrderUiStore(s => s.setShowTabsPanel)
  const setIsTabManagerExpanded = useOrderUiStore(s => s.setIsTabManagerExpanded)
  const setShowTipAdjustment = useOrderUiStore(s => s.setShowTipAdjustment)
  const setTabsRefreshTrigger = useOrderUiStore(s => s.setTabsRefreshTrigger)
  const setResendModal = useOrderUiStore(s => s.setResendModal)
  const setResendNote = useOrderUiStore(s => s.setResendNote)
  const setResendLoading = useOrderUiStore(s => s.setResendLoading)
  const setShowReceiptModal = useOrderUiStore(s => s.setShowReceiptModal)
  const setReceiptOrderId = useOrderUiStore(s => s.setReceiptOrderId)
  const setPreloadedReceiptData = useOrderUiStore(s => s.setPreloadedReceiptData)
  const setShowTabNamePrompt = useOrderUiStore(s => s.setShowTabNamePrompt)
  const setTabNameCallback = useOrderUiStore(s => s.setTabNameCallback)
  const setShowItemTransferModal = useOrderUiStore(s => s.setShowItemTransferModal)
  const setShowTabTransferModal = useOrderUiStore(s => s.setShowTabTransferModal)
  const setEditingNotesItemId = useOrderUiStore(s => s.setEditingNotesItemId)
  const setEditingNotesText = useOrderUiStore(s => s.setEditingNotesText)
  const setShowTimeClockModal = useOrderUiStore(s => s.setShowTimeClockModal)
  const setCurrentShift = useOrderUiStore(s => s.setCurrentShift)
  const setShowShiftStartModal = useOrderUiStore(s => s.setShowShiftStartModal)
  const setShowShiftCloseoutModal = useOrderUiStore(s => s.setShowShiftCloseoutModal)
  const tabsRefreshTrigger = useOrderUiStore(s => s.tabsRefreshTrigger)
  const resendModal = useOrderUiStore(s => s.resendModal)
  const resendNote = useOrderUiStore(s => s.resendNote)
  const editingNotesItemId = useOrderUiStore(s => s.editingNotesItemId)
  const editingNotesText = useOrderUiStore(s => s.editingNotesText)
  ...
  // ── Modifier prefetch cache (hover-to-prefetch for faster modal open) ──
  const modifierPrefetchCache = useRef(new Map<string, Promise<any[]>>())

  const prefetchModifierGroups = useCallback((menuItemId: string) => {
    if (modifierPrefetchCache.current.has(menuItemId)) return
    const promise = fetch(`/api/menu/items/${menuItemId}/modifier-groups`)
      .then(r => r.ok ? r.json() : null)
      .then(data => data?.data || [])
      .catch(() => null)
    modifierPrefetchCache.current.set(menuItemId, promise)
  }, [])

  const getCachedModifierGroups = useCallback(async (menuItemId: string): Promise<any[] | null> => {
    const cached = modifierPrefetchCache.current.get(menuItemId)
    if (cached) {
