...
run_schema() {
  ...
  # ── Build step ──
  # Docker-first model: the app is pre-built inside the container image.
  # No host-side npm run build — all builds happen in CI (GitHub Actions → GHCR).
  # The .next directory, server.js, and all assets are inside the Docker image.
  log "Docker-first model — no host build step (app runs inside container)"
